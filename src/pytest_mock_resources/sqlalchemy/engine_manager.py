...
from pytest_mock_resources.sqlalchemy import compat
from pytest_mock_resources.sqlalchemy.actions import AbstractAction, Rows, StaticStatements

StaticAction = Union[MetaData, compat.DeclarativeMeta, Rows, StaticStatements]
Action = Union[MetaData, compat.DeclarativeMeta, AbstractAction]
            ...
            if self.session:
                if isinstance(self.session, (sessionmaker, AsyncSession)):
                    ...
                    session_factory = sessionmaker(
                        ...
                        class_=compat.asyncio.AsyncSession,
                    )
...
def normalize_actions(ordered_actions: Iterable[T]) -> Iterable[T]:
    ...
    for action in ordered_actions:
        if isinstance(action, compat.DeclarativeMeta):
...
def identify_matching_tables(metadata, table_specifier):
    if isinstance(table_specifier, compat.DeclarativeMeta):
...
def create_async_engine(credentials, isolation_level=None):
    ...
    return compat.asyncio.create_async_engine(url, **options)
