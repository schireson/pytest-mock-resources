...
from pytest_mock_resources.resource.redshift.s3_copy import strip_query
...
def _parse_s3_command(statement):
    ...
    statement = strip_query(statement)
    params = dict()
    ...
    params["s3_uri"] = strip_query(tokens.pop(0))
    ...
    params["gzip"] = False
    for token in tokens:
        if strip_query(token.lower()) == "gzip":
            params["gzip"] = True
        ...
        if token.lower() == "delimiter":
            try:
                if tokens[index + 1].lower() != "as":
                    params["delimiter"] = strip_query(tokens[index + 1])
                else:
                    params["delimiter"] = strip_query(tokens[index + 2])
            except IndexError:
                raise ValueError(
                    (
