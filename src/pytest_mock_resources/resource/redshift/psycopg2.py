...
from pytest_mock_resources.compat import try_import
from pytest_mock_resources.config import RedshiftConfig
from pytest_mock_resources.resource.redshift.s3_copy import mock_s3_copy_command, strip_query
from pytest_mock_resources.resource.redshift.s3_unload import mock_s3_unload_command


@contextlib.contextmanager
def patch_connect(config: RedshiftConfig, database: str):
...
def mock_psycopg2_connect(config: RedshiftConfig, database: str, _connect):
    ...
    class CustomCursor(psycopg2.extensions.cursor):
        ...
        def execute(self, sql, args=None):
            ...
            if strip_query(sql).lower().startswith("copy"):
                mock_s3_copy_command(sql, self)
                sql = "commit"

            if strip_query(sql).lower().startswith("unload"):
